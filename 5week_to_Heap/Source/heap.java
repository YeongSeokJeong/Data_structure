...
public class heap {
	...
	public heap(int[] tree) {
		nNode = tree.length - 1;//마지막 노드를 가리키는 포인터
		maxSize = tree.length;//힙의 최대 사이즈
		heap = tree;//힙이 저장하는 숫자 배열
		int p, temp;
		for (int i = (int) (nNode / 2); i >= 1; i--) {//힙을 만들기 위해서는 내부노드부터 접근한다.
			p = i;//포인터는 내부노드를 관찰
			for (int j = p * 2; j <= nNode; j *= 2) {//해당 순번에 온 노드와 그 자식노드를 위한 조건문
				if (j < nNode) {//자식노드가 nNode보다 작으면 형재 노드가 있는것
					if (heap[j] < heap[j + 1])//형재노드와 크기를 비교하여 j가 큰 노드를 가리키게 한다.
						j ++;
				}
				if (heap[p] >= heap[j]) {//힙이 정상적으로 구현이 되었으면 반복문을 종료한다.
					break;
				}//크기 비교를 하여 부모와 자식 노드간의 값을 바꾸는 과정
				temp = heap[p];
				heap[p] = heap[j];
				heap[j] = temp;
				p = j;
